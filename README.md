...
Gpxgen injests the input file, then generates a gpx file for a looped run along the path of the input file. See next section for options.
...
|infile|`-i`|value of infile variable|pass this option to set the input file|
|padding|`-p`|1|pad the input route by interpolating additional points between every pair of successive points during processing. useful for converting bike rides to runs|
|loop|`-l`|loop|in the default behaviour, gpxgen appends to the input a mirror copy of itself during processing, so that the route forms a loop. set `-l` to disable this behaviour|
...
|reps|`-x`|1|number of times to run through the input route|
